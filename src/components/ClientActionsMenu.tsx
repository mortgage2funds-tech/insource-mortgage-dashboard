...
export default function ClientActionsMenu({ clientId, isArchived, role }: Props) {
  ...
  const [busy, setBusy] = useState<'archive' | 'unarchive' | 'delete' | null>(
    null
  );
  ...
  async function unarchiveClient() {
    try {
      ...
      alert(`Could not unarchive: ${e.message ?? e}`);
    } finally {
      setBusy(null);
    }
  }
  ...
  return (
    <div className="relative">
      <details ref={detailsRef} className="group">
        <summary
          className="cursor-pointer select-none rounded-xl border px-3 py-1 text-sm hover:shadow data-[busy=true]:opacity-60"
          data-busy={!!busy}
        >
          {busy ? 'Working…' : 'Actions'}
        </summary>

        <div className="absolute right-0 z-10 mt-2 hidden w-56 rounded-xl border bg-white p-2 shadow-lg group-open:block">
          {!isArchived ? (
            <button
              onClick={async () => {
                closeMenu(); // close instantly for UX
                await archiveClient();
              }}
              disabled={!!busy}
          ...
          {canHardDelete && (
            <>
              ...
              <button
                onClick={async () => {
                  // only close if they confirm; hardDeleteClient handles confirm()
